...
global TimeStruc inner_f outer_f inner_w PreTime2 
...
ro_outer_g,mu_outer_g,D_h_inner,D_h_outer,P1_inner,P2_inner,P1_outer,A1_inner,A1_outer,A_inner_wall,A_annulus]=varargin{:};
...
for i=1:mesh
    ...
    y(j)= 0.5 * (  -inner_f.P(i+1)+inner_f.P(i)-1e-3*dz*P1_inner/A1_inner*((f_inner/4) * G_inner^2/(2*ro_inner))...
                  -TimeStruc.inner.P(i+1)+TimeStruc.inner.P(i)-1e-3*dz*P1_inner/A1_inner*((f_inner/4) * G_inner^2/(2*ro_inner))  );
    ...
    y(k)= 0.5 * ( -inner_f.T(i+1)+ppval(splined_hl_T_coeff,inner_f.h(i+1))...
                 -TimeStruc.inner.T(i+1)+ppval(splined_hl_T_coeff,TimeStruc.inner.h(i+1))) ;
end
...
for i=1:mesh
    ...
    y(j)= 0.5 * (-outer_f.m(i+1)+outer_f.m(i)-(A_annulus*dz/dt)*(ro_tp-TimeStruc.outer.ro_tp(i))...
                 -TimeStruc.outer.m(i+1)+TimeStruc.outer.m(i)-(A_annulus*dz/dt)*(ro_tp-TimeStruc.outer.ro_tp(i)));
    ...
    y(k)= 0.5 * ( -outer_f.T(i+1)+ppval(splined_P_T_coeff,outer_f.P(i+1))...
                  -TimeStruc.outer.T(i+1)+ppval(splined_P_T_coeff,TimeStruc.outer.P(i+1)));

    hl_outer=ppval(splined_T_hl_coeff,outer_f.T(i+1));
    hg_outer=ppval(splined_T_hg_coeff,outer_f.T(i+1));
    
    y(kk)= 0.5 * ( -outer_f.x(i+1)+(outer_f.h(i+1)-hl_outer)/(hg_outer-hl_outer)...
                   -TimeStruc.outer.x(i+1)+(TimeStruc.outer.h(i+1)-hl_outer)/(hg_outer-hl_outer));
    PreTime2.hl_outer(i+1)=hl_outer;PreTime2.hg_outer(i+1)=hg_outer;
end
...
for i=1:mesh
    j=5*mesh+i;
      ...
      if i==1
        y(j)= 0.5 * (-a(i)*inner_w.T(i)+b*inner_w.T(i+1)+d(i)...
                     -a(i)*TimeStruc.wall.T(i)+b*TimeStruc.wall.T(i+1)+d(i));
      elseif i==mesh
        y(j)= 0.5 * (-a(i)*inner_w.T(i)+c*inner_w.T(i-1)+d(i)...
                     -a(i)*TimeStruc.wall.T(i)+c*TimeStruc.wall.T(i-1)+d(i));
      else
        y(j)= 0.5 * (-a(i)*inner_w.T(i)+b*inner_w.T(i+1)+c*inner_w.T(i-1)+d(i)...
                     -a(i)*TimeStruc.wall.T(i)+b*TimeStruc.wall.T(i+1)+c*TimeStruc.wall.T(i-1)+d(i));       
      end
