...
while CRITERIA > 10^-3
    ...
    for i=mesh+1:(N_r-1)*mesh
        ...
        wall_matrix(i,i)= thermal_cond_ins* ( 2*A_ew/dz + (A_s+A_n)/dr2 );         %  thermal_cond_ins*P2_outer*dz/dr1;
        wall_matrix(i,i-mesh)=-thermal_cond_ins*A_s/dr2;
        wall_matrix(i,i+mesh)=-thermal_cond_ins*A_n/dr2;
        if counter<mesh
            wall_matrix(i,i+1)=-thermal_cond_ins*A_ew/dz;
        end
        wall_vector(i)=0;   %0.5*(outer_f.T(i)+outer_f.T(i+1))*outer_f.hConv(i)*P1_outer*dz;
        counter=counter+1;
        if counter>mesh
            counter=1;
            j=j+1;
    ...
    for i=(N_r-1)*mesh+1:N_r*mesh
        ...
        wall_vector(i)=outer_w.hNat(i-(j-1)*mesh)*P3_outer*dz*T_inf; %   0.5*(outer_f.T(i)+outer_f.T(i+1))*outer_f.hConv(i)*P1_outer*dz;
        counter=counter+1;
    end
