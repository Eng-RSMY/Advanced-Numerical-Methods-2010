        ...
        while ERROR>10^-4
            ...
            if i<=floor(2*mesh/3)
                outer_f.h(i+1)=outer_f.h(i)+ 1e-3*(  outer_f.hConv(i)*P2_inner*dz*(inner_w.T(i)-T_mean)/(G_outer*A1_outer)...
                              - 0.5*(all_vg(3)^2-all_vg(1)^2) + (k_outer_g*A1_outer)*(outer_f.T(i+2)-2*outer_f.T(i+1)+outer_f.T(i))/(dz*G_outer*A1_outer)  );
            else
                outer_f.h(i+1)=outer_f.h(i)+ 1e-3*(  outer_f.hConv(i)*P2_inner*dz*(inner_w.T(i)-T_mean)/(G_outer*A1_outer)...
                              - 0.5*(all_vg(3)^2-all_vg(1)^2) + (k_outer_g*A1_outer)*(outer_f.T(i)-2*outer_f.T(i-1)+outer_f.T(i-2))/(dz*G_outer*A1_outer)  );
            end

            outer_f.T(i+1)=outer_f.T(i+1)*(outer_f.h(i+1)/previous_h)^2;
...
time=toc
plot((0:mesh)*dz,inner_f.T);
hold on
plot((0:mesh)*dz,outer_f.T);
hold on
plot((0:mesh-1)*dz+dz/2,inner_w.T);
