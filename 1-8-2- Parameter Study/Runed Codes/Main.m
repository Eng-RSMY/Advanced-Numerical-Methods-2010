...
%Mu_L=@(T)( 0.000000032451574*T.^4  -0.000009061289916 *T.^3+  0.000984509345712 *T.^2 -0.055211010387099*T +  1.778370453327188).*1e-3;
Mu_L=@(T)(0.8e-3);  %the mu is set to be constant during this study.
...
ro_inner=990; %kg/m3 average
mu_inner=0.8 *10^-3; %centipoise to SI

ro_outer_l=(ppval(splined_T_vl_coeff,T_outer_fluid))^-1; %kg/m3 average
mu_outer_l=0.2 *10^-3; %centipoise to SI
ro_outer_g=(ppval(splined_T_vg_coeff,T_outer_fluid))^-1; %kg/m3
...
while CRITERIA > 10^-5
    ...
    for i=1:mesh
       ...
       inner_f.P(i+1)=inner_f.P(i)-1e-3*dz/A1_inner*( P1_inner*(f_inner/4) * G_inner^2/(2*ro_inner) + ro_inner*A1_inner*g*sin(teta));
       ERROR=1;
       while ERROR>1e-5
            temp=inner_f.h(i+1);
...
%                                                 + G_outer*(all_kapa(3)-all_kapa(1)));
            ...
            outer_f.P(i+1)=outer_f.P(i)- 1e-3*dz/A_annulus*(phi_friedel*f_outer*G_outer^2*P2_inner/(8*ro_tp)+...
                                                     ro_tp*A_annulus*g*sin(teta)+G_outer*A_annulus*(all_kapa(3)-all_kapa(1))/dz);                                            
    ...
    %% inner wall simulation (one dimentional)
...
inner_f.T
subplot(3,2,2)
plot((0:mesh)*dz,inner_f.P,'color',col(col_count),'linewidth',line_wid,'marker',Marker(col_count),'markersize',Markersize,'markerfacecolor',col(col_count))
xlabel('length');ylabel('Subcooled water Pressure(kpa)');
hold on;
subplot(3,2,4)
plot((0:mesh)*dz,inner_f.T,'color',col(col_count),'linewidth',line_wid,'marker',Marker(col_count),'markersize',Markersize,'markerfacecolor',col(col_count))
xlabel('length');ylabel('Subcooled water Temperature(c)');
hold on;
subplot(3,2,1)
plot((0:mesh)*dz,outer_f.P,'color',col(col_count),'linewidth',line_wid,'marker',Marker(col_count),'markersize',Markersize,'markerfacecolor',col(col_count))
xlabel('length');ylabel('Steam Pressure(kpa)');
hold on;
subplot(3,2,3)
plot((0:mesh)*dz,outer_f.T,'color',col(col_count),'linewidth',line_wid,'marker',Marker(col_count),'markersize',Markersize,'markerfacecolor',col(col_count))
xlabel('length');ylabel('Steam Temperature(c)');
hold on;
subplot(3,2,5)
plot((0:mesh-1)*dz,1-outer_f.void,'color',col(col_count),'linewidth',line_wid,'marker',Marker(col_count),'markersize',Markersize,'markerfacecolor',col(col_count))
xlabel('length');ylabel('Liquid Film');
hold on;
end
col_count=1;
subplot(3,2,6)
plot(1,1,'color',col(col_count),'linewidth',line_wid,'visible','off','marker',Marker(col_count),'markersize',Markersize,'markerfacecolor',col(col_count))
hold on;col_count=2;
plot(2,2,'color',col(col_count),'linewidth',line_wid,'visible','off','marker',Marker(col_count),'markersize',Markersize,'markerfacecolor',col(col_count))
col_count=3;
plot(3,3,'color',col(col_count),'linewidth',line_wid,'visible','off','marker',Marker(col_count),'markersize',Markersize,'markerfacecolor',col(col_count))
col_count=4;
plot(4,4,'color',col(col_count),'linewidth',line_wid,'visible','off','marker',Marker(col_count),'markersize',Markersize,'markerfacecolor',col(col_count))
col_count=5;
plot(5,5,'color',col(col_count),'linewidth',line_wid,'visible','off','marker',Marker(col_count),'markersize',Markersize,'markerfacecolor',col(col_count))
axis off
 
legend([prop_name num2str(Prop(1))],[prop_name num2str(Prop(2))],[prop_name num2str(Prop(3))],[prop_name num2str(Prop(4))]...
        ,[prop_name num2str(Prop(5))],'location','north');
