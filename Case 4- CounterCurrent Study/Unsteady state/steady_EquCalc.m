function y = steady_EquCalc(X,varargin)
global  inner_f outer_f
...
ro_outer_g,mu_outer_g,D_h_inner,D_h_outer,P1_inner,P2_inner,P1_outer,A1_inner,A1_outer,A_inner_wall,thermal_cond_wall]=varargin{:};
...
for i=1:mesh
    j= 2*i - 1;
    k= 2*i;
    landa=mesh+2-i; %i was changed to handa and i+1 was changed to landa-1;
    ...
    inner_f.h(landa-1)=inner_f.h(landa)+ 1e-3*(inner_f.hConv(landa)*P1_inner*dz*(inner_w.T(landa-1)-T_mean)/(G_inner*A1_inner));
